...
class DomainViewModel @Inject constructor(
    ...
    private val _uiState = MutableStateFlow<UiState<DomainInfo>>(UiState.Idle)
    val uiState: StateFlow<UiState<DomainInfo>> = _uiState
    ...
    fun analyze() {
        ...
        val domain = _inputText.value.trim()
        if (domain.isBlank()) return
        viewModelScope.launch {
            _uiState.value = UiState.Loading
            _uiState.value = repository.analyzeDomain(domain).fold(
